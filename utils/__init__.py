...
def ensure_path(path, remove=True):
    ...
    if os.path.exists(path): #判断路径是否存在
        if remove and (basename.startswith('_') #检查字符串是否是以指定子字符串开头
                or input('{} exists, remove? ([y]/n): '.format(path)) != 'n'):
            shutil.rmtree(path) #递归的删除目录和文件
            os.makedirs(path)
    else:
...
def make_optimizer1(params, name, lr, weight_decay=None, milestones=False):
    if weight_decay is None:
        weight_decay = 0.
    if name == 'sgd':
        optimizer = SGD(filter(lambda p: p.requires_grad, params), lr, momentum=0.9, weight_decay=False)
    elif name == 'adam':
...
def make_optimizer(params, name, lr, weight_decay=None, milestones=True):
    if weight_decay is None:
        weight_decay = 0.
    if name == 'sgd':
        optimizer = SGD(filter(lambda p: p.requires_grad, params), lr, momentum=0.9, weight_decay=False)
